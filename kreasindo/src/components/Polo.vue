  ...
  <div class="polo-order">
    <div class="container">
      <div class="header">
        ...
        <h1 class="title">Pemesanan Kaos Polo</h1>
        <p class="subtitle">Pilih Kaos polo berkualitas sesuai kebutuhan Anda</p>
      </div>

      <div class="product-polo">
        <img src="/produk/polo.png" alt="Polo" />
        <div class="polo-overlay">
          <p>Kaos polo dengan tampilan rapi dan bahan nyaman, cocok untuk kebutuhan kerja, event, maupun seragam usaha.</p>
        </div>
      </div>
      ...
      <div v-for="(order, index) in orders" :key="index" class="order-card">
        ...
        <div class="card-body">
          ...
          <div class="form-group" v-if="order.size">
            ...
            <p class="helper-text" v-if="order.production === 'custom'">Minimal total kuantitas keseluruhan 12 pcs</p>
  ...
  methods: {
    ...
    sendToWhatsApp() {
      ...
      this.orders.forEach((order, index) => {
        ...
        message += `Jenis Kaos Polo: ${this.categoryOptions[order.category].name}\n`;
        message += `Tipe Kaos Polo: ${this.typeOptions[order.type].name}\n`;
        message += `Ukuran: ${order.size}\n`;
        message += `Kuantitas: ${order.quantity} pcs\n`;
        message += `Harga Kaos Polo: ${this.formatCurrency(order.unitPrice * order.quantity)}\n`;
