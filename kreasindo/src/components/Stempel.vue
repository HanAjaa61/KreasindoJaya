  ...
  <div class="stempel-order">
    <div class="container">
      <div class="header">
        ...
        <h1 class="title">Pemesanan Stempel</h1>
        <p class="subtitle">Pilih stempel berkualitas sesuai kebutuhan Anda</p>
      </div>

      <div class="product-banner">
        <img src="/produk/stempel.png" alt="Stempel" />
        <div class="banner-overlay">
          <p>Alat legalitas administrasi yang praktis dengan hasil cap yang tajam, tersedia dalam berbagai bentuk dan ukuran.</p>
        </div>
      </div>
        ...
        <div class="card-body">
            ...
            <div class="material-grid">
              ...
              >
                ...
                <div class="material-info">
                  ...
                  <p class="material-price">{{ formatCurrency(material.price) }}</p>
                  <div class="material-badge" v-if="material.design">+ Custom Desain</div>
                </div>
          ...
          <div class="form-row two-cols">
            <div class="form-group">
              ...
              <p v-else-if="isLockedSize(order.material)" class="helper-text">Ukuran untuk produk ini</p>
              <p v-else class="helper-text">Ukuran standar: 3 cm - 5 cm</p>
            </div>
...
export default {
  ...
  data() {
    return {
      ...
      materialOptions: {
        warnasatu: {
          name: 'Stempel 1 Warna',
          price: 100000,
          design: false
        },
        warnadua: {
          name: 'Stempel 2 Warna',
          price: 120000,
          design: false
        },
        warnatiga: {
          name: 'Stempel 3 Warna',
          price: 140000,
          design: false
        },
        legalisir: {
          name: 'Stempel Legalisir',
          price: 180000,
          design: false
        },
        medium: {
          name: 'Stempel Permanen Medium',
          price: 150000,
          design: false,
          fixedSize: 4
        },
        large: {
          name: 'Stempel Permanen Large',
          price: 200000,
          design: false,
          fixedSize: 5
        }
  ...
  methods: {
    ...
    validateSize(index) {
      ...
      if (size < 3) {
        order.sizeError = 'Ukuran kurang dari standar (minimal 3 cm)';
      } else if (size > 5) {
        order.sizeError = 'Ukuran melebihi standar (maksimal 5 cm)';
      } else {
        order.sizeError = '';
      }
    ...
    selectMaterial(index, key) {
      ...
      if (key === 'large') {
        this.orders[index].size = 5;
      } else if (key === 'medium') {
        this.orders[index].size = 4;
      }
    ...
    calculatePrice(index) {
      ...
      const basePrice = this.materialOptions[order.material].price;
      order.unitPrice = basePrice;
    ...
    sendToWhatsApp() {
      ...
      for (let i = 0; i < this.orders.length; i++) {
        ...
        if (!order.material || !order.size || !order.quantity) {
          this.showError('Data Pesanan Tidak Lengkap', `Mohon lengkapi data pesanan #${i + 1}! Pastikan jenis produk, ukuran, dan kuantitas sudah diisi.`);
          return;
        }
      }

      let message = `*PESANAN STEMPEL*\n\n`;
...
.input-with-unit .input-field.size-input:disabled {
  background: #e5e7eb;
  color: #6b7280;
  cursor: not-allowed;
  border-color: #d1d5db;
...
.input-with-unit .input-field.size-input:disabled + .unit {
  color: #9ca3af;
}
.address-note {
  margin-top: 8px;
