  ...
  <div class="plang-order">
    <div class="container">
      <div class="header">
        ...
        <h1 class="title">Pemesanan Plang</h1>
        <p class="subtitle">Pilih plang berkualitas sesuai kebutuhan Anda</p>
      </div>

      <div class="product-plang">
        <img src="/produk/plang.png" alt="Plang" />
        <div class="plang-overlay">
          <p>Plang adalah representasi visual sebuah lokasi atau usaha, dibuat dengan material berkualitas dan desain yang dirancang agar tetap terlihat jelas dan profesional.</p>
        </div>
      </div>
        ...
        <div class="card-body">
          ...
          <div class="form-group">
            ...
            <p class="address-note">
              <strong>Catatan :</strong> Alamat akan digunakan untuk survey lokasi pemasangan plang.
            </p>
          </div>
        </div>
      ...
      <div v-for="(order, index) in orders" :key="index" class="order-card">
        ...
        <div class="card-body">
            ...
            <div class="material-grid">
              ...
              >
                ...
                <div class="material-info">
                  ...
                  <p class="material-price">{{ formatCurrency(material.pricePerM2) }} / m²</p>
                </div>
          ...
          <div class="form-group">
            <label>Ukuran Plang <span class="required">*</span></label>
            <div class="dimension-inputs">
              <div class="dimension-input-group">
                <label class="dimension-label">Panjang (meter)</label>
                <input 
                  v-model.number="order.length" 
                  type="number" 
                  min="0.1"
                  step="0.1"
                  ...
                  :disabled="!order.material"
                />
              </div>
              <div class="dimension-separator">×</div>
              <div class="dimension-input-group">
                <label class="dimension-label">Lebar (meter)</label>
                <input 
                  v-model.number="order.width" 
                  type="number" 
                  min="0.1"
                  step="0.1"
                  class="dimension-input"
            ...
            <div class="dimension-info">
              <p class="helper-text">Pilih jenis produk terlebih dahulu. Minimal {{ order.material === 'flexi' ? '1' : '0.5' }} m²</p>
              <p v-if="order.length && order.width" class="area-display">
                Luas: <strong>{{ (order.length * order.width).toFixed(2) }} m²</strong>
              </p>
            </div>
          </div>

          <div class="form-group">
            <label class="main-label">Tambahan (Opsional)</label>
            
            <div class="addition-section">
              <h4 class="addition-title">Pemasangan Lampu</h4>
              <div class="addition-grid">
                <div 
                  v-for="(lamp, key) in lampOptions" 
                  :key="key"
                  class="addition-card"
                  :class="{ 'selected': order.lamp === key }"
                  @click="selectLamp(index, key)"
                >
                  <div class="addition-info">
                    <h5 class="addition-name">{{ lamp.name }}</h5>
                    <p class="addition-price">{{ formatCurrency(lamp.price) }}</p>
                  </div>
                  <div class="check-icon-small" v-if="order.lamp === key">
            ...
            <div class="addition-section">
              <h4 class="addition-title">Biaya Pasang</h4>
              <div class="addition-single">
                <div 
                  class="addition-card"
                  :class="{ 'selected': order.installation }"
                  @click="selectInstallation(index)"
                >
                  <div class="addition-info">
                    <h5 class="addition-name">Jasa Pemasangan</h5>
                    <p class="addition-price">{{ formatCurrency(installationPrice) }}</p>
                  </div>
                  <div class="check-icon-small" v-if="order.installation">
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3">
                      <polyline points="20 6 9 17 4 12"></polyline>
                    </svg>
                  </div>
          ...
          <div class="screenshot-guide" v-if="order.productPrice">
      ...
      <span>Catatan: Pesanan tanpa lampiran screenshot yang valid dapat ditolak atau dengan kesepakatan bersama !</span>
          ...
          <div class="price-summary">
            <div class="price-row">
              ...
              <span class="price-value">{{ formatCurrency(order.productPrice) }}</span>
            </div>
            <div v-if="order.lamp" class="price-row">
              <span class="price-label">Lampu ({{ lampOptions[order.lamp].name }}):</span>
              <span class="price-value">{{ formatCurrency(lampOptions[order.lamp].price) }}</span>
            </div>
            <div v-if="order.installation" class="price-row">
              <span class="price-label">Biaya Pasang:</span>
              <span class="price-value">{{ formatCurrency(installationPrice) }}</span>
            </div>
...
export default {
  ...
  data() {
    return {
      ...
      lampOptions: {
        lamp20: {
          name: 'Lampu Sorot 20 Watt',
          price: 350000
        },
        lamp30: {
          name: 'Lampu Sorot 30 Watt',
          price: 500000
        }
      },
  ...
  methods: {
    generateOrderCode() {
      ...
      this.orderCode = `PlG${randomNum}05`;
    },
    saveCustomerData() {
    ...
    selectMaterial(index, key) {
      ...
      this.orders[index].length = 0;
      this.orders[index].width = 0;
      this.calculatePrice(index);
    },
    selectLamp(index, key) {
      if (this.orders[index].lamp === key) {
        this.orders[index].lamp = null;
      } else {
        this.orders[index].lamp = key;
      }
      this.calculatePrice(index);
    },
    selectInstallation(index) {
      this.orders[index].installation = !this.orders[index].installation;
      this.calculatePrice(index);
    },
    addOrder() {
      this.orders.push({
    ...
    calculatePrice(index) {
      ...
      if (!order.material || !order.length || !order.width) {
        order.productPrice = 0;
        order.totalPrice = 0;
        return;
      }

      const material = this.materialOptions[order.material];
      const area = order.length * order.width;
      
      if (area < material.minArea) {
        order.productPrice = material.minPrice;
      } else {
        order.productPrice = Math.round(area * material.pricePerM2);
      }
      
      let total = order.productPrice;
      
      if (order.lamp) {
        total += this.lampOptions[order.lamp].price;
      }
      
      if (order.installation) {
        total += this.installationPrice;
      }
    ...
    sendToWhatsApp() {
      ...
      for (let i = 0; i < this.orders.length; i++) {
        ...
        if (!order.material || !order.length || !order.width) {
          this.showError('Data Pesanan Tidak Lengkap', `Mohon lengkapi data pesanan #${i + 1}! Pastikan jenis produk dan ukuran sudah diisi!`);
          return;
        }
      }

      let message = `*PESANAN PLANG*\n\n`;
      ...
      this.orders.forEach((order, index) => {
        const area = (order.length * order.width).toFixed(2);
        message += `\n*Pesanan #${index + 1}*\n`;
        message += `*Kode Pesanan: ${this.orderCode}*\n`;
        message += `Jenis Produk: ${this.materialOptions[order.material].name}\n`;
        message += `Ukuran: Panjang ${order.length}m x Lebar ${order.width}m (${area} m²)\n`;
        message += `Harga Produk: ${this.formatCurrency(order.productPrice)}\n`;
        
        if (order.lamp) {
          message += `Pemasangan Lampu: ${this.lampOptions[order.lamp].name} (${this.formatCurrency(this.lampOptions[order.lamp].price)})\n`;
        }
        
        if (order.installation) {
          message += `Biaya Pasang: ${this.formatCurrency(this.installationPrice)}\n`;
        }
      ...
      message += `\nSegera konfirmasi saya terkait ketersediaan produk atau hal-hal lainnya. Terima kasih!`;
...
.addition-section:first-of-type {
  margin-top: 20px;
}
...
.addition-card.selected .addition-name {
  color: #fff;
}
...
.addition-card.selected .addition-price {
  color: #fff;
}
...
.dialog-btn-cancel:hover {
  transform: translateY(-3px);
  box-shadow: 0 10px 24px rgba(255, 68, 68, 0.45);
}
