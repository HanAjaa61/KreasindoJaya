  ...
  <div class="luster-order">
    <div class="container">
      <div class="header">
        ...
        <h1 class="title">Pemesanan Luster</h1>
        <p class="subtitle">Pilih jenis luster berkualitas sesuai kebutuhan Anda</p>
      </div>

      <div class="product-banner">
        <img src="/produk/luster.png" alt="Luster" />
        <div class="banner-overlay">
          <p>Cetak foto berkualitas tinggi dengan tekstur halus dan efek semi-gloss yang memberikan kesan mewah dan elegan.</p>
        </div>
      </div>
        ...
        <div class="card-body">
            ...
            <div class="material-grid">
              ...
              >
                ...
                <div class="material-info">
                  ...
                  <p class="material-price">{{ formatCurrency(product.price) }} / m²</p>
                </div>
          ...
          <div class="form-row two-cols">
            <div class="form-group size-group">
              <label>Panjang (meter) <span class="required">*</span></label>
              <div class="input-with-unit">
                <input 
                  ...
                  :class="{ 'error': order.lengthError }"
                  @input="validateAndCalculate(index)"
                />
                <span class="unit">meter</span>
              </div>
              <p v-if="order.lengthError" class="error-message">{{ order.lengthError }}</p>
              <p v-else class="helper-text">Minimal 1 meter</p>
            </div>

            <div class="form-group size-group">
              <label>Lebar (meter) <span class="required">*</span></label>
              <div class="input-with-unit">
                <input 
                  v-model="order.width" 
                  type="number" 
                  ...
                  placeholder="1.0"
                  class="input-field size-input"
                  :class="{ 'error': order.widthError }"
                  @input="validateAndCalculate(index)"
                />
                <span class="unit">meter</span>
              </div>
              <p v-if="order.widthError" class="error-message">{{ order.widthError }}</p>
              <p v-else class="helper-text">Min 0.5m (50cm) - Maks 1.5m (150cm)</p>
            </div>
          </div>
          ...
          <div class="screenshot-guide" v-if="order.product">
    ...
    <div class="guide-footer">
      <span>Catatan: Pesanan tanpa lampiran screenshot yang valid dapat ditolak atau dengan kesepakatan bersama !</span>
          ...
          <div class="price-summary">
            <div class="price-row">
              <span class="price-label">Luas Area:</span>
              <span class="price-value">{{ order.area ? order.area.toFixed(2) : '0' }} m²</span>
            </div>
            <div class="price-row">
...
export default {
  ...
  methods: {
    generateOrderCode() {
      ...
      this.orderCode = `LsR${randomNum}02`;
      },
    saveCustomerData() {
    ...
    selectProduct(index, key) {
      ...
      this.validateAndCalculate(index);
    },
    addOrder() {
      this.orders.push({
        product: '',
    ...
    validateAndCalculate(index) {
      const order = this.orders[index];
      let hasError = false;

      const length = parseFloat(order.length);
      if (order.length && !isNaN(length)) {
        if (length < 1) {
          order.lengthError = 'Panjang minimal 1 meter';
          hasError = true;
        } else {
          order.lengthError = '';
        }
      ...
      if (order.width && !isNaN(width)) {
        if (width < 0.5) {
          order.widthError = 'Lebar minimal 0.5 meter (50 cm)';
          hasError = true;
        } else if (width > 1.5) {
          order.widthError = 'Lebar maksimal 1.5 meter (150 cm)';
          hasError = true;
        } else {
          order.widthError = '';
        }
    ...
    calculatePrice(index) {
      ...
      if (!order.product || !order.length || !order.width || !order.quantity) {
        order.area = 0;
        order.unitPrice = 0;
        order.totalPrice = 0;
        return;
      ...
      let length = parseFloat(order.length);
      ...
      const pricePerMeter = this.productOptions[order.product].price;
      order.unitPrice = pricePerMeter * area;
    ...
    sendToWhatsApp() {
      ...
      for (let i = 0; i < this.orders.length; i++) {
        ...
        if (!order.product || !order.length || !order.width || !order.quantity) {
          this.showError('Data Pesanan Tidak Lengkap', `Mohon lengkapi data pesanan #${i + 1}! Pastikan jenis, panjang, lebar, dan kuantitas sudah diisi.`);
          return;
        }
        
        if (order.lengthError || order.widthError) {
          this.showError('Data Pesanan Tidak Valid', `Mohon perbaiki ukuran pada pesanan #${i + 1}!`);
          return;
        }
      }

      let message = `*PESANAN LUSTER*\n\n`;
      ...
      this.orders.forEach((order, index) => {
        ...
        message += `Ukuran: Panjang ${order.length}m x Lebar ${order.width}m (${order.area.toFixed(2)} m²)\n`;
