  ...
  <div class="stikermeteran-order">
    <div class="container">
      <div class="header">
        ...
        <h1 class="title">Pemesanan Stiker Meteran</h1>
        <p class="subtitle">Pilih stiker meteran berkualitas sesuai kebutuhan Anda</p>
      </div>

      <div class="product-banner">
        <img src="/produk/stikeroneway.png" alt="Stiker Meteran" />
        <div class="banner-overlay">
          <p>Solusi branding skala besar untuk kaca atau kendaraan (termasuk oneway vision) dengan ukuran custom sesuai kebutuhan.</p>
        </div>
      </div>
        ...
        <div class="card-body">
            ...
            <div class="material-grid">
              ...
              >
                ...
                <div class="material-info">
                  ...
                  <p class="material-price">{{ formatCurrency(material.price) }} / m²</p>
          ...
          <div class="form-row two-cols">
            <div class="form-group">
              ...
              <p class="helper-text">Bebas tanpa batasan</p>
            </div>
            ...
            <div class="form-group">
              ...
              <p v-else class="helper-text">Maksimal 150 meter</p>
            </div>
          </div>
...
export default {
  ...
  data() {
    return {
      ...
      materialOptions: {
        onewaynopasang: {
          name: 'Stiker Oneway (Tanpa Pemasangan)',
          price: 150000,
          design: true
        },
        onewaypasang: {
          name: 'Stiker Oneway (Dengan Pemasangan)',
          price: 250000,
          design: true
        }
      },
      materialNames: {
        onewaynopasang: 'Stiker Oneway (Tanpa Pemasangan)',
        onewaypasang: 'Stiker Oneway (Dengan Pemasangan)',
      }
    }
  },
  computed: {
  ...
  methods: {
    generateOrderCode() {
      ...
      this.orderCode = `SmR${randomNum}03`;
      },
    saveCustomerData() {
    ...
    validateAndCalculatePrice(index) {
      ...
      if (order.width) {
        ...
        if (width > 150) {
          order.width = 150;
          order.widthError = 'Lebar maksimal 150 meter';
          setTimeout(() => {
            order.widthError = '';
          }, 2000);
        } else {
    ...
    calculatePrice(index) {
      ...
      if (!order.material || !order.length || !order.width || !order.quantity) {
        order.unitPrice = 0;
        order.totalPrice = 0;
        return;
      }
      ...
      const pricePerMeter = this.materialOptions[order.material].price;
      order.unitPrice = pricePerMeter * totalArea;
    ...
    sendToWhatsApp() {
      ...
      for (let i = 0; i < this.orders.length; i++) {
        ...
        if (!order.material || !order.length || !order.width || !order.quantity) {
          this.showError('Data Pesanan Tidak Lengkap', `Mohon lengkapi data pesanan #${i + 1}! Pastikan bahan, panjang, lebar, dan kuantitas sudah diisi!`);
          return;
        }
      }

      let message = `*PESANAN STIKER METERAN*\n\n`;
      ...
      this.orders.forEach((order, index) => {
        const totalArea = parseFloat(order.length) * parseFloat(order.width);
        
        message += `\n*Pesanan #${index + 1}*\n`;
        message += `*Kode Pesanan: ${this.orderCode}*\n`;
        message += `Jenis Produk: ${this.materialNames[order.material]}\n`;
        message += `Panjang: ${order.length} meter\n`;
        message += `Lebar: ${order.width} meter\n`;
        message += `Total Luas: ${totalArea.toFixed(2)} m²\n`;
