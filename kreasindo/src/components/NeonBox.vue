  ...
  <div class="neon-order">
    <div class="container">
      <div class="header">
        ...
        <h1 class="title">Pemesanan Neon Box</h1>
        <p class="subtitle">Pilih neon box berkualitas sesuai kebutuhan Anda</p>
      </div>

      <div class="product-neon">
        <img src="/produk/neonbox.png" alt="Neon Box" />
        <div class="neon-overlay">
          <p>Neon Box membantu bisnis anda tampil lebih menonjol terutama pada malam hari melalui media promosi visual yang menarik, informatif, dan dirancang agar efektif menjangkau target pasar.</p>
        </div>
      </div>
        ...
        <div class="card-body">
          ...
          <div class="form-group">
            ...
            <p class="address-note">
              <strong>Catatan :</strong> Alamat akan digunakan untuk survey lokasi pemasangan neon box.
            </p>
          </div>
        </div>
      ...
      <div v-for="(order, index) in orders" :key="index" class="order-card">
        ...
        <div class="card-body">
          ...
          <div class="form-group">
            ...
            <p class="helper-text">Pilih jenis produk terlebih dahulu. Minimal 1 piece</p>
          </div>
            ...
            <div class="addition-section">
              ...
              <div class="addition-grid">
                <div 
                  ...
                  :key="key"
                  class="addition-card"
                  :class="{ 'selected': order.pole === key }"
                  @click="selectPole(index, key)"
                >
                  <div class="addition-info">
                    <h5 class="addition-name">{{ pole.name }}</h5>
                    <p class="addition-price">{{ formatCurrency(pole.price) }}</p>
                  </div>
                  <div class="check-icon-small" v-if="order.pole === key">
              ...
              <div class="addition-grid">
                ...
                >
                  <div class="addition-info">
                    ...
                    <p class="addition-price">{{ formatCurrency(installation.price) }}</p>
                  </div>
                  <div class="check-icon-small" v-if="order.installation === key">
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3">
                      <polyline points="20 6 9 17 4 12"></polyline>
                    </svg>
                  </div>
          ...
          <div class="price-summary">
            <div class="price-row">
              ...
              <span class="price-value">{{ order.material === 'custom' ? 'Hubungi Kami' : formatCurrency(order.productPrice) }}</span>
            </div>
            <div v-if="order.pole" class="price-row">
              <span class="price-label">Tiang ({{ poleOptions[order.pole].name }}):</span>
              <span class="price-value">{{ formatCurrency(poleOptions[order.pole].price) }}</span>
            </div>
            <div v-if="order.installation" class="price-row">
              <span class="price-label">Pemasangan ({{ installationOptions[order.installation].name }}):</span>
              <span class="price-value">{{ formatCurrency(installationOptions[order.installation].price) }}</span>
            </div>
            <div class="price-row total-row">
              <span class="price-label">Total Harga:</span>
              <span class="price-value total">{{ order.material === 'custom' ? 'Hubungi Kami' : formatCurrency(order.totalPrice) }}</span>
            </div>
          </div>
        </div>
      </div>
        ...
        <div class="grand-total-section">
          <div class="total-content">
            ...
            <div class="total-info">
              ...
              <p class="total-amount">{{ hasCustomOrder ? 'Hubungi Kami' : formatCurrency(grandTotal) }}</p>
            </div>
          </div>
        </div>
...
export default {
  ...
  computed: {
    ...
    hasCustomOrder() {
      return this.orders.some(order => order.material === 'custom');
    }
  },
  mounted() {
    window.scrollTo(0, 0);
  ...
  methods: {
    generateOrderCode() {
      ...
      this.orderCode = `NeB${randomNum}01`;
    },
    saveCustomerData() {
    ...
    calculatePrice(index) {
      ...
      const materialPrice = this.materialOptions[order.material].price;
      order.productPrice = materialPrice * order.quantity;
      
      let total = order.productPrice;
      
      if (order.pole) {
        total += this.poleOptions[order.pole].price;
      }
      
      if (order.installation) {
        total += this.installationOptions[order.installation].price;
      }
    ...
    sendToWhatsApp() {
      ...
      this.orders.forEach((order, index) => {
        ...
        message += `Kuantitas: ${order.quantity} piece\n`;
        
        if (order.material === 'custom') {
          message += `Harga Produk: Mohon konfirmasi untuk harga custom\n`;
        } else {
          message += `Harga Produk: ${this.formatCurrency(order.productPrice)}\n`;
        }
        
        if (order.pole) {
          message += `Tambahan Tiang: ${this.poleOptions[order.pole].name} (${this.formatCurrency(this.poleOptions[order.pole].price)})\n`;
        }
        
        if (order.installation) {
          message += `Jasa Pemasangan: ${this.installationOptions[order.installation].name} (${this.formatCurrency(this.installationOptions[order.installation].price)})\n`;
        }
        
        if (order.material === 'custom') {
          message += `Total: Mohon konfirmasi\n`;
        } else {
      ...
      if (this.hasCustomOrder) {
        message += `*TOTAL KESELURUHAN: Mohon konfirmasi untuk harga custom*\n\n`;
      } else {
...
.material-card.selected .material-custom-note {
  color: #fff;
}
