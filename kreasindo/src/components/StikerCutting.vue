<template>
  <div class="cutting-stiker-order">
    <div class="container">
      <div class="header">
        ...
        <h1 class="title">Pemesanan Cutting Stiker</h1>
        <p class="subtitle">Pilih produk stiker berkualitas sesuai kebutuhan Anda</p>
      </div>

      <div class="product-banner">
        <img src="/produk/cuttingstiker.png" alt="Cutting Stiker" />
        <div class="banner-overlay">
          <p>Stiker potong presisi untuk logo kendaraan atau dekorasi sunblast kaca yang estetis untuk privasi ruangan.</p>
        </div>
      </div>
      ...
      <div v-for="(order, index) in orders" :key="index" class="order-card">
        ...
        <div class="card-body">
            ...
            <div class="material-grid">
              ...
              >
                ...
                <div class="material-info">
                  ...
                  <p class="material-price">{{ material.priceText }}</p>
                </div>
          ...
          <div v-if="order.material === 'cutting'" class="form-row two-cols">
            <div class="form-group">
              <label>Ukuran (cm) <span class="required">*</span></label>
              <div class="input-with-unit">
                <input 
                  ...
                  :class="{ 'error': order.sizeError }"
                  @input="validateSize(index)"
                />
                <span class="unit">cm</span>
              </div>
              <p v-if="order.sizeError" class="error-message">{{ order.sizeError }}</p>
              <p v-else class="helper-text">Minimal 1 cm (bilangan bulat)</p>
            </div>
          ...
          <div v-if="order.material === 'nomor'" class="form-row two-cols">
            <div class="form-group">
              ...
              <input 
                ...
                disabled
              />
              <p class="helper-text">Ukuran standar nomor lambung</p>
            </div>
          ...
          <div v-if="order.material === 'sunblast' || order.material === 'sunblastPasang'" class="form-row two-cols">
            <div class="form-group">
              <label>Ukuran (meter) <span class="required">*</span></label>
              <div class="input-with-unit">
                <input 
                  v-model="order.size" 
                  type="number" 
              ...
              <p v-if="order.sizeError" class="error-message">{{ order.sizeError }}</p>
              <p v-else class="helper-text">Minimal 1 meter</p>
            </div>
...
export default {
  ...
  data() {
    return {
      ...
      materialOptions: {
        ...
        nomor: {
          name: 'Nomor Lambung',
          priceText: 'Rp 50.000 / piece',
          pricePerUnit: 50000
        },
        sunblast: {
          name: 'Stiker Sunblast Non Pasang',
          priceText: 'Rp 180.000 / meter',
          pricePerUnit: 180000
        },
        sunblastPasang: {
          name: 'Stiker Sunblast + Pasang',
          priceText: 'Rp 250.000 / meter',
          pricePerUnit: 180000
        }
      },
      materialNames: {
        ...
        nomor: 'Nomor Lambung',
        sunblast: 'Stiker Sunblast Non Pasang',
        sunblastPasang: 'Stiker Sunblast + Pasang'
      }
    }
  },
  computed: {
  ...
  methods: {
    generateOrderCode() {
      ...
      this.orderCode = `ScT${randomNum}11`;
    },
    ...
    validateSize(index) {
      ...
      if (!order.size || isNaN(size)) {
        order.sizeError = '';
        this.calculatePrice(index);
        return;
      }

      if (order.material === 'cutting') {
        if (size < 1) {
          order.sizeError = 'Ukuran minimal 1 cm';
        } else if (!Number.isInteger(size)) {
          order.sizeError = 'Ukuran harus bilangan bulat';
        } else {
          order.sizeError = '';
        }
      } else if (order.material === 'sunblast' || order.material === 'sunblastPasang') {
        if (size < 1) {
          order.sizeError = 'Ukuran minimal 1 meter';
        } else {
          order.sizeError = '';
        }
      }
    ...
    selectMaterial(index, key) {
      ...
      this.orders[index].sizeError = '';
      
      if (key === 'nomor') {
        this.orders[index].size = '35x15';
      }
    ...
    calculatePrice(index) {
      ...
      if (order.material === 'cutting') {
        ...
        if (!isNaN(size) && size >= 1) {
          order.unitPrice = material.pricePerUnit * size;
          order.totalPrice = order.unitPrice * order.quantity;
        } else {
          order.unitPrice = 0;
          order.totalPrice = 0;
        }
      } else if (order.material === 'nomor') {
        order.unitPrice = material.pricePerUnit;
        order.totalPrice = order.unitPrice * order.quantity;
      } else if (order.material === 'sunblast') {
        const size = parseFloat(order.size);
        if (!isNaN(size) && size >= 1) {
          order.unitPrice = material.pricePerUnit * size;
          order.totalPrice = order.unitPrice * order.quantity;
      ...
      } else if (order.material === 'sunblastPasang') {
        const size = parseFloat(order.size);
        if (!isNaN(size) && size >= 1) {
          order.unitPrice = (material.pricePerUnit * size) + 70000;
          order.totalPrice = order.unitPrice * order.quantity;
        } else {
          order.unitPrice = 0;
          order.totalPrice = 0;
    ...
    sendToWhatsApp() {
      ...
      for (let i = 0; i < this.orders.length; i++) {
        ...
        if (!order.material) {
          this.showError('Data Pesanan Tidak Lengkap', `Mohon pilih jenis produk pada pesanan #${i + 1}!`);
          return;
        }
        
        if (order.material !== 'nomor' && (!order.size || parseFloat(order.size) < 1)) {
          this.showError('Data Pesanan Tidak Lengkap', `Mohon isi ukuran pada pesanan #${i + 1}!`);
          return;
        }
        
        if (!order.quantity || order.quantity < 1) {
          this.showError('Data Pesanan Tidak Lengkap', `Mohon isi kuantitas pada pesanan #${i + 1}!`);
          return;
        }
      }

      let message = `*PESANAN CUTTING STIKER*\n\n`;
      ...
      this.orders.forEach((order, index) => {
        ...
        if (order.material === 'cutting') {
          message += `Ukuran: ${order.size} cm\n`;
        } else if (order.material === 'nomor') {
          message += `Ukuran: 35x15 cm\n`;
        } else if (order.material === 'sunblast' || order.material === 'sunblastPasang') {
          message += `Ukuran: ${order.size} meter\n`;
        }
...
.input-with-unit .input-field.size-input {
  width: 100%;
  padding-right: 70px;
}
